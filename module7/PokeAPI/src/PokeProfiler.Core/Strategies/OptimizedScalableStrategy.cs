...
namespace PokeProfiler.Core.Strategies
{
    public class OptimizedScalableStrategy(PokeApiClient client) : IPokemonFetchStrategy
    {
        ...
        async Task<IReadOnlyList<Pokemon>> IPokemonFetchStrategy.FetchAsync(IEnumerable<string> idsOrNames, CancellationToken ct)
        {
            ...
            var optimalConcurrency = Math.Min(Environment.ProcessorCount * 2, 16);
            ...
            // Use Partitioner with load balancing for better work distribution
            ...
            Parallel.ForEach(partitions, 
                ...
                id =>
                {
                    var task = FetchPokemonAsync(id, ct, results, activity);
                    tasks.Add(task);
                });
        ...
        private async Task FetchPokemonAsync(string id, CancellationToken ct, ConcurrentQueue<Pokemon> results, System.Diagnostics.Activity? parentActivity)
        {
            ...
            catch (Exception ex)
            {
                ...
                parentActivity?.SetStatus(System.Diagnostics.ActivityStatusCode.Error, ex.Message);
            }
        }
    }
}
